 ...
 *
 * Щит смещён вниз так, что все вершины ниже плеч уходят за нижний край
 * карточки: в кадре остаются верхушки и прямые линии, а не большое скругление.
 */
...
export const SCENE = {
  ...
  heroWidth: 520,
  /*
   * Поле из четырнадцати контуров. Начинаем с 360: контуры уже настолько
   * велики, что их верхушки распределяются по карточке, а все нижние вершины
   * лежат за её краем — ровно этого мы и добивались.
   */
  field: Array.from({ length: 14 }, (_, i) => 360 + i * 32),
} as const;

/** Проверка, что нижние вершины контура ушли за край карточки. */
export function hipY(width: number) {
  const height = width * SHIELD_RATIO;
  return SCENE.cy - height / 2 + height * HIP_RATIO;
}
